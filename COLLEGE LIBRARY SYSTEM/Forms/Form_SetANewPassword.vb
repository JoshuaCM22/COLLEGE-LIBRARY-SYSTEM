...
Public Class Form_SetANewPassword
    ...
    Private Sub SettingANewPassword()
        Try
            con.Open()
            cmd = New OleDbCommand("UPDATE [tbl_admins] set [password]=@password WHERE [ID]=@ID", con) With {
                .CommandType = CommandType.Text
            }
            cmd.Parameters.Add(New OleDbParameter("@password", BCrypt.Net.BCrypt.HashPassword(txtConfirmNewPassword.Text)))
            cmd.Parameters.Add(New OleDbParameter("@ID", userIDGetter))
            cmd.ExecuteNonQuery()
            cmd.Dispose()
